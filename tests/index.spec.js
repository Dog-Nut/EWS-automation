...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Enter PIN' }).fill(process.env.PRINTER_PIN);
  await page.getByRole('textbox', { name: 'Enter PIN' }).press('Enter');
  await expect(page.getByRole('button', { name: 'Sign In/Sign Out' })).toBeVisible();
  ...
  await expect(page.locator('mat-grid-tile:nth-child(2) > .mat-grid-tile-content').first()).toBeVisible();
  await expect(page.locator('mat-grid-tile:nth-child(3) > .mat-grid-tile-content')).toBeVisible();
  await expect(page.locator('mat-grid-tile:nth-child(4) > .mat-grid-tile-content')).toBeVisible();
});